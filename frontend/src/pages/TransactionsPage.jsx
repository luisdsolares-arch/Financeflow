...
export default function TransactionsPage() {
  ...
  const exportCsv = async () => {
    try {
      ...
      setStatus("No se pudo exportar el CSV de transacciones.");
    }
  };
    ...
    <div className="space-y-4 sm:space-y-5">
      <div className="panel p-4 sm:p-5 lg:p-6">
        <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
          ...
          <button onClick={exportCsv} className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-100 sm:text-sm">Exportar CSV</button>
        </div>
      </div>
        ...
        <div className="panel p-4 sm:p-5 lg:p-6">
          ...
          <div className="mt-3 space-y-3">
            {plannedExpenses.length ? plannedExpenses.map((item) => (
              <article key={item.id} className="rounded-2xl border border-slate-200 p-3 sm:p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-semibold text-slate-800">{item.description}</p>
                    <p className="text-xs text-slate-500 sm:text-sm">{item.category} · {item.recurrence_type === "one_time" ? "Puntual" : item.recurrence_type === "weekly" ? "Recurrente semanal" : "Recurrente mensual"}</p>
                  </div>
                  <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">{formatCurrency(item.amount)}</span>
                </div>
                <div className="mt-3 grid gap-2 text-xs text-slate-600 sm:text-sm md:grid-cols-2">
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id} className="border-b border-slate-100">
                  <td className="py-2 sm:py-3">{row.date}</td>
